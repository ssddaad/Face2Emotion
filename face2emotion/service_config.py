...
def _validate(config: ServiceConfig) -> ServiceConfig:
    ...
    if not (0.1 <= config.micro_roi_brow_weight <= 5.0):
        raise ValueError("micro_roi_brow_weight 必须在 [0.1, 5.0]")
    if not (0.1 <= config.micro_roi_eye_weight <= 5.0):
        raise ValueError("micro_roi_eye_weight 必须在 [0.1, 5.0]")
    if not (0.1 <= config.micro_roi_mouth_weight <= 5.0):
        raise ValueError("micro_roi_mouth_weight 必须在 [0.1, 5.0]")
    ...
    if config.motion_complexity not in (0, 1, 2):
        raise ValueError("motion_complexity 必须是 0/1/2")
    if config.motion_interval < 1:
        raise ValueError("motion_interval 必须 >= 1")
    if not (0.01 <= config.motion_landmark_ema_alpha <= 1.0):
        raise ValueError("motion_landmark_ema_alpha 必须在 [0.01, 1.0]")
    return config
...
def load_service_config(config_path: str | None = None) -> ServiceConfig:
    ...
    config = ServiceConfig(
        ...
        micro_roi_brow_weight=float(_env("F2E_MICRO_ROI_BROW") or _pick(raw, "micro_roi_brow_weight", 1.25)),
        micro_roi_eye_weight=float(_env("F2E_MICRO_ROI_EYE") or _pick(raw, "micro_roi_eye_weight", 1.45)),
        micro_roi_mouth_weight=float(_env("F2E_MICRO_ROI_MOUTH") or _pick(raw, "micro_roi_mouth_weight", 1.60)),
        ...
        enable_motion_capture=_as_bool(
            _env("F2E_MOTION_CAPTURE") or _pick(raw, "enable_motion_capture", False), False
        ),
        motion_complexity=int(_env("F2E_MOTION_COMPLEXITY") or _pick(raw, "motion_complexity", 1)),
        motion_vote_window=int(_env("F2E_MOTION_VOTE_WINDOW") or _pick(raw, "motion_vote_window", 10)),
        motion_interval=int(_env("F2E_MOTION_INTERVAL") or _pick(raw, "motion_interval", 1)),
        motion_landmark_ema_alpha=float(
            _env("F2E_MOTION_LM_ALPHA") or _pick(raw, "motion_landmark_ema_alpha", 0.45)
        ),
    )
